...
Le lot `content_pipeline` publie son manifeste et 1 000 quêtes générées, identiques octet pour octet entre les sources serveur et client auditées. Les marqueurs tels que `balance_validated` ou `ready_for_import` décrivent uniquement l'état déclaré par la source et ne constituent pas une certification. Les 168 sorts générés restent exclus comme instantanés, mais sont désormais couverts par 168 identités originales et statless reliées uniquement par empreinte privée ; aucun nom, effet, formule, groupe, culture, asset ou contrôle source n'est repris.
...
Les cinq petites familles à droits mixtes sont entièrement exclues comme instantanés après audit, soit 114 fichiers. `bulk_orders` reprend une mécanique fortement associée à Ultima Online et son fichier de récolte contient en plus une référence tierce. Les 17 `talent` forment un arbre très proche de Warcraft. `racial_traits` contient des contrôles admin/debug, des assets et un fichier One Piece. `race` combine imports Sphere, Skaven, Tiefling, Drow, clans Vampire, contrôles internes et assets. Enfin, les 35 fichiers source `skills` restent exclus pour contrôles admin/debug, références tierces, état runtime, commerce ou assets ; même les sept fichiers structurellement génériques conservent des marqueurs de debug. Le registre publie séparément douze compétences originales et portables, sans prétendre qu'elles sont des copies de ces sources.
...
Les 155 fichiers source de la famille `item` restent exclus comme instantanés : 151 entrées contiennent économie, contrôles internes, état runtime, assets ou référence tierce. `sets.json` et `wings.json` sont des sérialisations de tableaux .NET sans contrat de modèle exploitable. Les deux exemples de conscience d'objet restants mélangent une valeur économique aux propriétés physiques et contiennent du texte mojibake. Le registre publie séparément 147 adaptations originales et portables : six objets fictifs et 141 adaptations liées à une source `item`. Ces adaptations retirent prix, stock, boutique, effets, chemins d'assets et contrôles internes ; elles ne sont pas des copies source et ne prouvent aucune compatibilité runtime. Les quatorze autres sources demeurent explicitement exclues pour droits ou références tierces ; aucune source `item` ne reste en attente d'adaptation.

Les 428 fichiers source de la famille `monsters` restent exclus comme instantanés. Tous relèvent du contrat interne/admin ; 426 exposent directement `admin_controls` et `debug_mode`. Le lot contient aussi un dragon UO, un chemin d'asset et quatre instantanés runtime de plantes. Le registre publie séparément 428 identités originales et statless, dérivées de catégories génériques et reliées uniquement par empreinte SHA-256 privée. Aucun nom, chemin source, statistique, capacité, butin, comportement IA, asset ou contrôle n'est repris. Aucune source `monsters` ne reste non traitée.

Les 1 615 fichiers source de la famille `spells` restent exclus comme instantanés. Tous exposent `admin_controls` et `debug_mode`, 14 ajoutent `admin_command`, et la collection mélange notamment des groupes One Piece, Aion, Warcraft et des panthéons réels. Trois sorts ajoutent des signaux commerciaux et un autre un mot-clé sensible. Le registre publie séparément 1 615 identités originales et statless reliées uniquement par empreinte SHA-256 privée. Avec les 168 adaptations de `content_pipeline/spells`, les 1 783 sources de sorts sont entièrement traitées sans reprendre aucun nom, chemin, effet, formule, groupe, culture, asset ou contrôle. Huit anciennes entrées publiques non conformes, qui exposaient encore `admin_controls` ou `debug_mode`, ont été retirées du registre local.
...
L'inventaire agrégé couvre les trois sources et distingue les snapshots publiés des adaptations originales. Les 4 009 sources serveur courantes sont toutes comptabilisées : 1 180 par empreinte de snapshot et 2 829 par empreinte d'adaptation publique. Aucun fichier ne reste en attente, exclu sans représentation ou invalide. Les chemins sensibles restent dans une liste de travail privée ; les comptes agrégés et motifs de revue sont publics dans [AUDIT-COVERAGE.json](AUDIT-COVERAGE.json).

Un état `pending-review` n'est pas une validation. La couverture est déclarée complète uniquement après inventaire réel, validation du catalogue, vérification des empreintes, scan global des motifs interdits et exécution des tests de la pipeline.
