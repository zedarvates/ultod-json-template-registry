...
from scripts.cohort_six_contracts import COHORT_SIX_FAMILIES, KIND_BY_FAMILY, STRICT_FAMILIES, candidate_slug, convert_legacy, family_schema
from scripts.pilot_contracts import MigrationResult
from scripts.template_contract import decode_json_bytes


def _render(value): return (json.dumps(value, indent=2, ensure_ascii=False) + "\n").encode("utf-8")


def _readme(family):
    return (f"# {family.title()} templates\n\n## Purpose\n\nMinimal declarative {family} identity and classification.\n\n## Required fields\n\n`display_name`; masterpieces may include `profession_tag`.\n\n## Authoritative exclusions\n\nExcluded: stats, difficulty values, experience, materials, result items, skill caps, skill lists, narrative beats, outcomes, points, tiers, rewards, buffs, and runtime rules.\n\n## Intended consumers\n\nLLM pipeline only. Zig support requires separate typed-adapter evidence.\n\n## Compatibility evidence\n\nNone verified.\n\n## Versioning\n\nTemplates and family schemas follow independent SemVer.\n").encode("utf-8")


def build_migration(root: Path, families=COHORT_SIX_FAMILIES):
    if tuple(families) != COHORT_SIX_FAMILIES: raise ValueError("fixed cohort")
    catalog=decode_json_bytes((root/"templates/catalog.json").read_bytes()); strict_set=set(STRICT_FAMILIES)
    ...
    for family in families:
        paths=sorted(p.relative_to(root).as_posix() for p in (root/"templates"/family).rglob("template.json") if p.parent.name!="v1.0.0"); decoded={}; candidates=[]
        for source in paths:
            try: d=decode_json_bytes((root/source).read_bytes()); decoded[source]=d; candidates.append(candidate_slug(source,d))
            except Exception: pass
        collisions={s for s,c in Counter(candidates).items() if c>1}
        for source in paths:
            ...
            content=_render(result.document); files[result.target_file]=content; legacy.update({"id":f"{family}:{result.slug}","slug":result.slug,"family":family,"superseded_by":f"{family}:{result.slug}@1.0.0"})
        ...
        sp=f"templates/schemas/{family}/v1.0.0/schema.json"; sc=_render(family_schema(family)); files[sp]=sc; entries.append({"name":family,"kind":"json-schema","version":"1.0.0","status":"experimental","file":sp,"sha256":hashlib.sha256(sc).hexdigest(),"compatibility":[],"validation_profile":"strict-schema-v1","contract_version":"1.0.0"}); files[f"templates/{family}/README.md"]=_readme(family)
    results.sort(key=lambda r:r.source_file); counts=dict(sorted(Counter(r.disposition for r in results).items())); report={"report_version":"1.0.0","cohort":"6","families":list(families),"summary":{"total":len(results),"dispositions":counts},"results":[{"source_file":r.source_file,"family":r.family,"slug":r.slug,"disposition":r.disposition,"reason_codes":list(r.reason_codes),"target_file":r.target_file} for r in results]}; catalog["entries"]=entries; files["MIGRATION-V1-COHORT-6.json"]=_render(report); files["templates/catalog.json"]=_render(catalog); return {"results":results,"counts":counts,"files":files}


def write_migration(root,plan):
    for rel,content in sorted(plan["files"].items()):
        p=root/rel; p.parent.mkdir(parents=True,exist_ok=True)
        if p.exists() and "v1.0.0/template.json" in rel and p.read_bytes()!=content: raise FileExistsError(rel)
