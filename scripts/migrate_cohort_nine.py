...
FAMILIES = ("rts", "divine-system")
NARRATIVE_SLUGS = {
    "buddhist-divine-system-completion-report",
    "class-integration-examples",
    "detailed-spell-examples",
    "divine-quest-storylines",
}
...
def build(root: Path):
    ...
    for family in FAMILIES:
        ...
        for source in paths:
            ...
            narrative = family == "divine-system" and slug in NARRATIVE_SLUGS
            results.append({
                "source_file": source,
                "family": family,
                "slug": slug,
                "disposition": "legacy-only-narrative" if narrative else "legacy-only-authoritative",
                "reason_codes": ["examples-report-or-storyline"] if narrative else ["aggregate-runtime-system-or-catalog"],
                "target_file": None,
            })
...
def main():
    ...
    output = args.root / "MIGRATION-V1-COHORT-9.json"
    if args.write:
