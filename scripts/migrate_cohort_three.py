...
from scripts.cohort_three_contracts import COHORT_THREE_FAMILIES, KIND_BY_FAMILY, candidate_slug, convert_legacy, family_schema
...
def _readme(family: str) -> bytes:
    return (
        ...
        "## Required fields\n\n`display_name`. Optional kind and category tags are defined by the exact schema.\n\n"
        "## Authoritative exclusions\n\nExcluded: stats, damage, healing, costs, cooldowns, range, progression, requirements, rewards, recipes, markets, effects, lore, dialogue, and asset paths.\n\n"
...
def build_migration(root: Path, families: tuple[str, ...] = COHORT_THREE_FAMILIES):
    if tuple(families) != COHORT_THREE_FAMILIES:
        raise ValueError("cohort three families are fixed for reproducible review")
    catalog = decode_json_bytes((root / "templates/catalog.json").read_bytes())
    family_set = set(families)
    entries = [
        entry for entry in catalog["entries"]
        if not (entry.get("validation_profile") == "strict-v1" and entry.get("family") in family_set)
        and not (entry.get("validation_profile") == "strict-schema-v1" and entry.get("name") in family_set)
    ]
    ...
    report = {
        "report_version": "1.0.0", "cohort": "3", "families": list(families),
        "summary": {"total": len(results), "dispositions": counts},
        "results": [{"source_file": r.source_file, "family": r.family, "slug": r.slug, "disposition": r.disposition, "reason_codes": list(r.reason_codes), "target_file": r.target_file} for r in results],
    }
    catalog["entries"] = entries
    files["MIGRATION-V1-COHORT-3.json"] = _render(report)
