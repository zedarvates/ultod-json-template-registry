...
from scripts.cohort_four_contracts import COHORT_FOUR_FAMILIES, KIND_BY_FAMILY, STRICT_FAMILIES, candidate_slug, convert_legacy, family_schema
...
def _readme(family: str) -> bytes:
    return (
        ...
        "## Required fields\n\n`display_name`. Optional kind, category, domain, alignment, school, and rarity tags are defined by the exact schema.\n\n"
        "## Authoritative exclusions\n\nExcluded: stats, costs, cooldowns, awareness, AI behavior, dialogue, schedules, loot, shops, permissions, standings, powers, blessings, curses, effects, lore, and asset paths.\n\n"
...
def build_migration(root: Path, families: tuple[str, ...] = COHORT_FOUR_FAMILIES):
    if tuple(families) != COHORT_FOUR_FAMILIES:
        raise ValueError("cohort four families are fixed for reproducible review")
    catalog = decode_json_bytes((root / "templates/catalog.json").read_bytes())
    strict_set = set(STRICT_FAMILIES)
    entries = [entry for entry in catalog["entries"] if not (entry.get("validation_profile") == "strict-v1" and entry.get("family") in strict_set) and not (entry.get("validation_profile") == "strict-schema-v1" and entry.get("name") in strict_set)]
    by_file = {entry["file"]: entry for entry in entries}
    results: list[MigrationResult] = []
    files: dict[str, bytes] = {}
    for family in families:
        paths = sorted(path.relative_to(root).as_posix() for path in (root / "templates" / family).rglob("template.json") if path.parent.name != "v1.0.0")
        decoded = {}
        candidates = []
        for source in paths:
            try:
        ...
        for source in paths:
            ...
            entries.append({
                ...
                "schema_file": f"templates/schemas/{family}/v1.0.0/schema.json", "file": result.target_file,
        ...
        entries.append({"name": family, "kind": "json-schema", "version": "1.0.0", "status": "experimental", "file": schema_path, "sha256": hashlib.sha256(schema_content).hexdigest(), "compatibility": [], "validation_profile": "strict-schema-v1", "contract_version": "1.0.0"})
    ...
    report = {"report_version": "1.0.0", "cohort": "4", "families": list(families), "summary": {"total": len(results), "dispositions": counts}, "results": [{"source_file": r.source_file, "family": r.family, "slug": r.slug, "disposition": r.disposition, "reason_codes": list(r.reason_codes), "target_file": r.target_file} for r in results]}
    catalog["entries"] = entries
    files["MIGRATION-V1-COHORT-4.json"] = _render(report)
