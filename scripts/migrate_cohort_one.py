...
from scripts.cohort_one_contracts import COHORT_ONE_FAMILIES, KIND_BY_FAMILY, candidate_slug, convert_legacy, family_schema
...
def _readme(family: str) -> bytes:
    fields = {
        "avatars": "`display_name`; optional species, variant, and body-frame classification tags.",
        "biomes": "`display_name`; optional category, climate, and terrain classification tags.",
        "currencies": "`display_name` and `currency_code`.",
        "names": "`display_name` and `culture_tag`.",
    }[family]
    excluded = "progression, rewards, stats, timings, probabilities, spawn rules, balances, lore, dialogue, and asset paths"
    return (
        f"# {family.title()} templates\n\n"
        f"## Purpose\n\nMinimal declarative {family} identity and classification.\n\n"
        f"## Fields\n\n{fields}\n\n"
        f"## Authoritative exclusions\n\nExcluded: {excluded}.\n\n"
...
def build_migration(root: Path, families: tuple[str, ...] = COHORT_ONE_FAMILIES):
    if tuple(families) != COHORT_ONE_FAMILIES:
        raise ValueError("cohort one families are fixed for reproducible review")
    catalog = decode_json_bytes((root / "templates/catalog.json").read_bytes())
    strict_families = set(families) - {"achievements"}
    entries = [
        entry
        for entry in catalog["entries"]
        if not (
            entry.get("validation_profile") == "strict-v1"
            and entry.get("family") in strict_families
        )
        and not (
            entry.get("validation_profile") == "strict-schema-v1"
            and entry.get("name") in strict_families
        )
    ]
    ...
    for family in families:
        ...
        collisions = {slug for slug, count in candidate_counts.items() if count > 1}
        for source in paths:
            try:
                document = decoded[source]
            ...
            entries.append({
                ...
                "intended_consumers": result.document["intended_consumers"],
                "compatibility": [],
        ...
        entries.append({
            ...
            "status": "experimental",
            "file": schema_path,
            "sha256": hashlib.sha256(schema_content).hexdigest(),
            "compatibility": [],
    ...
    files["MIGRATION-V1-COHORT-1.json"] = _render(report)
